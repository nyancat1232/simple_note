    ...
    for col_local_foreign in tss_foreign:
        ...
        with tab_or_col[col_local_foreign]:
            ...
            if len(selected_col_convert[col_local_foreign])>0:
                selections=dfs_foreign[col_local_foreign][selected_col_convert[col_local_foreign][0]].unique().dropna().tolist()
                custom_configs_rw_def[col_local_foreign+'__conversion']=st.column_config.SelectboxColumn(f'{col_local_foreign}(conversion from {selected_col_convert[col_local_foreign][0]})',
                                                                                                         options=selections)
